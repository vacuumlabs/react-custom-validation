...
function one(config, name, updateData) {
  let {onValidation, debounce = 100, validations} = config
...
export function validated(getConfig) {
  return (Component) => class extends React.Component {
    ...
    componentWillMount() {
      ...
      this.onFormValidProvider = new OnFormValidProvider(this.isFormValid)
    }
    ...
    updateData = (name, data) => {
      ...
      // onFormValidProvider may depend on hidden state (this.validationData),
    ...
    render() {
      return (<Component
        ...
        onFormValid={(h) => this.onFormValidProvider.onFormValid(h)}
      />)
    }
  }
}
