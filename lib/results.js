...
function setLastFnStart(state, name, when) {
  return update(state, {results: {[name]: {lastFnStart: {$set: when}}}})
}
...
function runFn(state, name, dispatchUpdate) {
  ...
  let r = lastFn()
  let s = setLastFnStart(state, name, new Date().getTime())
  if (!isPromise(r)) {
    return setResult(s, name, r)
  } else {
    r.then((rr) => {
  ...
  dispatchUpdate((state) => {
    ...
    for (let name in config.validations) {
      ...
      if (name in oldResults && isEqual(rules, oldResults[name].rules)) {
        ...
        let {lastFnStart = 0} = oldResults[name] || {}
        let result = {isValid: null, error: {}}

        newState = update(
          newState,
          {results: {[name]: {$set: {rules, debounce, lastFn, lastFnStart, result}}}}
        )

        let nextRunIn = lastFnStart + debounce - new Date().getTime()
