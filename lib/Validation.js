...
import {initValidation, eventType} from './helpers'
...
export class Validation {
  ...
  update({rules, fields, onValidation, onDestroy, name}) {
    for (let ruleName in rules) {
      ...
      if (this.rules[ruleName] != null && this.rules[ruleName].fn !== rules[ruleName].fn) {
        throw new Error(
          ...
          `different function anythime it is evaluated. Check the rule ${name}/${ruleName}.`)
      }
    }
