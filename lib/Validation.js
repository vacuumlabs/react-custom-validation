...
export class Validation {
  ...
  constructor(name, {rules, fields, onValidation, onDestroy}, fieldObserver) {
  ...
  destroy() {
    if (this.onDestroy) {
      ...
      this.handleValidation({result: {valid: true}, show: false})
    }
  ...
  update({rules, fields, onValidation, onDestroy}) {
    for (let ruleName in rules) {
      ...
      if (this.rules[ruleName] != null && this.rules[ruleName].fn !== rules[ruleName].fn) {
        throw new Error(
          `Changing \`fn\` for given rule is not allowed. Note that '() => {...}' creates a ` +
          `different function anytime it is evaluated. Check the rule ${this.name}/${ruleName}.`)
      }
    }

    if (!isEqual(this.rules, rules)) {
      this.handleValidation({result: {valid: null}}, onValidation)
